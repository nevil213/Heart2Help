...
function Settings({setUser}) {
    ...
    const values = {
        ...
        isAdult : JSON.parse(Cookies.get('data')).data.isAdult === 1 ? 'yes' : 'no',
        isHealthy : JSON.parse(Cookies.get('data')).data.isHealthy === 1 ? 'no' : 'yes',
        lastDonated : new Date(JSON.parse(Cookies.get('data')).data.lastDonated).toISOString().split('T')[0]
    }
