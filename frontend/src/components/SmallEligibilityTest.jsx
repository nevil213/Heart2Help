...
const SmallEligibilityTest = () => {
    ...
    const checkUserType = () => {
      try {
        ...
        const fetchDonorDetails = async () => {
          try {
            const response = await axios.post("http://localhost:8080/getDonorDetails", { user_id: answers.id });
  ...
  const handleSubmit = async (e) => {
    ...
    if (answers.lastDonated === 'never') {
      updatedAnswers.lastDonated = '1753-01-01';
    ...
    try {
      await axios.post("http://localhost:8080/quick_survey", updatedAnswers);

      const { isAdult, isHealthy, lastDonated } = updatedAnswers;
      const filteredData = { data: { isAdult, isHealthy, lastDonated } };
...
export default SmallEligibilityTest;